...
|---------|-------------------|----------|
| sibling skill not installed | `harden apply` returns PilotSubmissionError | `uv tool install "vmware-harden[remediation]==1.10.8"` — installing the sibling on its own lands in a separate tool env harden cannot import from |
