...
The `[collectors]` extra is what `scan` needs: inventory is read through vmware-aiops / vmware-storage / vmware-nsx-security, and `uv tool install` isolates each tool's environment — installing those packages on their own leaves harden unable to import them. Plain `uv tool install vmware-harden==1.10.10` is enough to report on an existing twin DB.
...
1. Install: `uv tool install "vmware-harden[collectors]==1.10.10"` (the extra carries the inventory collectors `scan` reads through)
